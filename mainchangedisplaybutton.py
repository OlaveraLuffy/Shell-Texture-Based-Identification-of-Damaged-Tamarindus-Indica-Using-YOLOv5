...
def last_img():
    ...
    if logic2 == 1:
        ...
        if initial_display == True:
            second2 = first2.resize((352,288), Image.Resampling.LANCZOS)
        else:
            second2 = first2.resize((160,145), Image.Resampling.LANCZOS)
        third2 = ImageTk.PhotoImage(second2)
        detectedLabel.third2 = third2  # keep a reference so it's not garbage collected
...
def change_display_mode():
    ...
    if initial_display:
        print('Camera')
        #insert code here if camera on main screen
        ...
        print('Detection')
        #insert code here if detection on main screen
        cameraLabel.place(x=355,y=15,width=160,height=145)
        detectedLabel.place(x=160,y=185,width=352,height=288)
        cap.set(3, 160)
        cap.set(4, 145)
        ts2 = 0
        logic2 = 0
...
changeDisplayButton = customtkinter.CTkButton(sidebar_frame, command=change_display_mode, text="Change Display")
changeDisplayButton.grid(row=2, column=0, padx=20, pady=10)
