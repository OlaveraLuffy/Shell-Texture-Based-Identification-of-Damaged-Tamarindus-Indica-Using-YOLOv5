...
def video_stream():
    ret, frame = cap.read()
    cv2.normalize(frame, frame, 0, 255, cv2.NORM_MINMAX)
    #res = cv2.resize(frame,(352,288), interpolation = cv2.INTER_CUBIC)
    #cv2image = cv2.cvtColor(res, cv2.COLOR_BGR2RGBA)
...
def last_img():
    ...
    if logic2 == 1:
        ...
        second2 = first2.resize((352,220), Image.Resampling.LANCZOS)
...
#Captured
captureFrame = customtkinter.CTkFrame(gui, width=540, height = 280)
captureFrame.grid(row=0, column=1, columnspan = 2, padx=10 , pady=3, sticky="nsew")
...
detectedLabel.place(x=160,y=5,width=352,height=220)

#Camera
cameraFrame = customtkinter.CTkFrame(gui, width =350, height=280)
cameraFrame.grid(row=1, column=1, columnspan =4, rowspan =3, padx=10, pady=3, sticky="nsew")
cameraLabel = tk.Label(gui)
cameraLabel["borderwidth"] = "1px"
cameraLabel["relief"] = "solid"
cameraLabel["text"] = "No image found"
cameraLabel.place(x=160,y=245, width=352, height=230)
